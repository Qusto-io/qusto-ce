...
test.describe('Loading States', () => {
  test('Dashboard shows loading indicator while fetching data', async ({
    page
  }) => {
    ...
    const loadingIndicators = page.locator(
      '[data-testid="loading"], [class*="spinner"], [class*="skeleton"], [aria-busy="true"]'
    )
  ...
  test('Revenue chart shows skeleton loader before data loads', async ({
    page
  }) => {
    ...
    const loader = page.locator(
      '[data-testid="chart-loading"], [class*="skeleton"]'
    )
  ...
  test('Infinite scroll shows loading indicator when fetching more data', async ({
    page
  }) => {
    await page.goto('/')
    ...
    if (await loadMore.isVisible()) {
      ...
      const loading = page.locator(
        '[data-testid="loading-more"], [class*="spinner"]'
      )
  ...
  test('Table shows loading state while fetching rows', async ({ page }) => {
    ...
    if ((await table.count()) > 0) {
      ...
      const tableLoading = page.locator(
        '[data-testid="table-loading"], tbody[class*="loading"]'
      )
  ...
  test('Page transition shows loading indicator', async ({ page }) => {
    ...
    if (await link.first().isVisible()) {
      ...
      const loader = page.locator(
        '[data-testid="page-loader"], [class*="page-loading"]'
      )
  ...
  test('Dashboard refreshes with loading indicator', async ({ page }) => {
    ...
    const refreshButton = page.locator(
      '[data-testid="refresh"], button:has-text("Refresh")'
    )
